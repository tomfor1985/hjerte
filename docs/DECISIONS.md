...
Updated 2026-09-06 after the inventory evaluation: the user requires automatic source approval without manual item review and approved Sol as the default inventory writer/checker, original figure images and bounded Astra repair. Inventory permits two repair rounds within the existing per-job cap (25 NOK default) and application allowance. Astra handles visual/difficult repairs and the last repair pass. Checked objectives are retained separately from complete section coverage. Unresolved claims stay held, with coverage gaps visible. Paid drafts may be reused only with verified matching source evidence and must receive the current independent check; no previous failure is relabelled as an approval.

Question generation remains Terra with separate Sol blind-answer and rationale checks. Evidence images required by mapped objectives are supplied during writing and both checks; they are also supplied during later matching. All models use high reasoning and Flex. API input token counting includes images before reserving generation costs; generation still fails closed if the model-specific count or the remaining reservation cannot be established. Uncertain API calls retain their reservation and are never automatically retried. No automatic library-wide continuation, allowance expansion, service-tier fallback or new provider has been authorized.
