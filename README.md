...
The completion estimate covers the active main guidelines and source-checking of their notes, with separate core coverage and useful-depth totals. It extrapolates unseen guideline content at 0.5–1.5 times the observed density of distinct matched objectives, after removing layout whitespace; this is an explicit scenario, not a confidence interval. Notes and alternate formats do not duplicate the objective count. It includes remaining mapping, matching, existing-question linking and production, and separately shows prior spend and uncertain reservations. The new flow needs a measured pilot; until then the estimate uses historical token repricing and a broad planning reserve. Missing source text or unmeasured required stages prevent a falsely complete amount.
