...
class CompletionEstimateTests(TestCase):
    ...
    def test_preview_is_private_and_does_not_spend_or_queue_anything(self):
        ...
        self.assertContains(response,'Coverage and cost')
        self.assertContains(response,'A reliable full-library cost is not established yet')
        self.assertContains(response,'depends on the remaining chapter coverage')
